...
var Cal2 = function () {
  ...
  function attachHandlers() {
    ...
    $('#pageCal2').on('change', '#cbShowTimes', function () {
      _calendarDiv.toggleClass('showTimes', !!$('#cbShowTimes').prop('checked'));
      // _calendarDiv.find('#cbShowTimes').blur();
    });
    $('#pageCal2').on('change', '#cbCal2Darker', function () {
      $('#pageCal2').toggleClass('darkerColors', !!$('#cbCal2Darker').prop('checked'));
  ...
  function buildMonth(bYear, bMonth) {
    ...
    var dayCellTemplate = [
        ...
        '<div class=dayCell>',
            '<div class=top><span class=dayNum>{^holyDayAftStar}{bDay}</span> <span class=sunsetStart>{frag1WeekdayShort}<span> {startingSunsetDesc}</span></span></div>',
                '<div class=night>',
