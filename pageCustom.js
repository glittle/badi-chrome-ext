...
var PageCustom = function () {
    ...
    function loadFormatsFromSync() {
        var localLoad = function () {
            chrome.storage.local.get({
                ...
                if (info.customFormats.length) {
                    console.log("formats loaded from local: ".concat(info.customFormats.length));
        ...
        if (browserHostType === browser.Chrome) {
            chrome.storage.sync.get({
                ...
                if (info.customFormats.length) {
                    console.log("formats loaded from sync: ".concat(info.customFormats.length));
                    recallSettings(info.customFormats);
                }
                else {
                    localLoad();
    ...
    function fillSelectDefault(id, storageId, message) {
        ...
        $.each(_itemsForDefaultSelects, function (i, el) {
            ...
            optionsHtml.push("<option value=\"".concat(format.replace(/"/g, "&quot;"), "\" data-prefix=\"").concat(isDefault ? getMessage("defaultFormat") : "").concat(el.letter, " - \" data-format=\"").concat(format, "\"></option>"));
        });
        ...
        if (formats.length > 0) {
            ...
            $.each(formats, function (i, el) {
                var format = el.f;
                optionsHtml.push("<option value=\"".concat(format.replace(/"/g, "&quot;"), "\" data-prefix=\"").concat(i + 1, " - \" data-format=\"").concat(format, "\"></option>"));
            });
            optionsHtml.push("</optgroup>");
        }
        // fill select
        var ddl = $("#".concat(id)).html((defaultFound
            ? ""
